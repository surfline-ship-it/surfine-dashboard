...
/**
 * Resolves which partner key to use for HubSpot / sheet data.
 * Non-admin: always JWT `partner`. Admin (`ALL`): `selectedPartner` query/body must match a credential partner.
 */
export function resolveDataPartner(payload, selectedPartnerRaw) {
  ...
  if (typeof jwtPartner !== "string" || !jwtPartner.trim()) {
    return { dataPartner: null, isAdmin: false, error: "Missing partner in token." };
  }
  if (jwtPartner !== ADMIN_JWT_PARTNER) {
    return { dataPartner: jwtPartner.trim(), isAdmin: false };
  }
  const requested =
  ...
  const allowed = new Set(getDistinctCredentialPartners());
  if (!allowed.has(requested)) {
    return { dataPartner: null, isAdmin: true, error: "Invalid selectedPartner." };
  }
  return { dataPartner: requested, isAdmin: true };
}
