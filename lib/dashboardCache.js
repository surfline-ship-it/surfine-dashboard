...
export function dashboardCacheVersion() {
  return process.env.CACHE_VERSION || process.env.VERCEL_GIT_COMMIT_SHA || "v1";
}

export function buildLeadsCacheKey(partner, searchFilter) {
  const searchPart = searchFilter || "all";
  return `${dashboardCacheVersion()}::${partner}::leads::${searchPart}`;
}
