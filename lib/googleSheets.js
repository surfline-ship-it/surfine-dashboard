...
const DEFAULT_OUTREACH_TAB = "All Campaigns - Instantly";
const OUTREACH_SHEET_CACHE_KEY = "__outreach_instantly_sheet_v2__";
...
function buildOutreachTabCandidates(tabProperties) {
  ...
  push(DEFAULT_OUTREACH_TAB);

  const byGid = (tabProperties || []).find(
    (t) => Number(t.sheetId) === OUTREACH_CAMPAIGNS_GID
  );
  push(byGid?.title);

  for (const t of tabProperties || []) {
    if (/instantly|campaign/i.test(String(t.title || ""))) {
      push(t.title);
    }
  }

  push("All Instantly Leads");
  return out;
}
