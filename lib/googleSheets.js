...
function findDashboardStatsHeaderRow(values) {
  for (let i = 0; i < values.length; i++) {
    const row = values[i] || [];
    const normalized = row.map(normalizeHeader);
    if (
...
function toDashboardStatsRow(row, headerMap) {
  const get = (name) => {
    const idx = headerMap.get(name);
    return idx == null ? "" : String(row[idx] || "").trim();
  };
  ...
  const searchName = canonicalSearchName(get("search name"));
  if (!partner || !searchName) return null;
  return {
    partner,
    searchName,
...
export async function readDashboardStatsRows() {
  ...
  const range = "Dashboard Stats!A1:G200";
  ...
  if (!res.ok) {
    const err = await res.text();
    throw new Error(`Google Sheets Dashboard Stats API failed: ${res.status} ${err}`);
  }
  const data = await res.json();
  const values = Array.isArray(data.values) ? data.values : [];
  const headerRowIndex = findDashboardStatsHeaderRow(values);
  if (headerRowIndex < 0) {
    throw new Error("Could not find Dashboard Stats header row");
  }
  ...
  const rows = [];
  for (let i = headerRowIndex + 1; i < values.length; i++) {
    const parsed = toDashboardStatsRow(values[i] || [], headerMap);
    if (parsed) rows.push(parsed);
  }
  return { sheetId: DASHBOARD_SUMMARY_SHEET_ID, rows };
}
