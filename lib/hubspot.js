...
function getDealStageEnteredAt(deal) {
  const p = deal.properties || {};
...
function formatStageEnteredLabel(value) {
  const ts = Date.parse(value || "");
...
function stageEnteredSortKey(deal) {
  const ts = Date.parse(getDealStageEnteredAt(deal) || "");
...
function buildPipelineProgression(filteredDeals) {
  ...
  const dealPayload = (d) => {
    const stageEnteredAt = getDealStageEnteredAt(d);
    return {
      id: d.id,
      ...
      stageEnteredLabel: formatStageEnteredLabel(stageEnteredAt),
    };
  };
  ...
  PIPELINE_PROGRESSION_ROW_ORDER.forEach((def) => {
    const deals = (buckets[def.rowId] || []).sort(
      (a, b) => stageEnteredSortKey(b) - stageEnteredSortKey(a)
    );
