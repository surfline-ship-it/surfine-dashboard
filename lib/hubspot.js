...
export const DEAL_STAGES = {
  // Investor Matching Pipeline
  "3471208175": { label: "Pre-Qual Booking", order: 1 },
  "3220310749": { label: "Prequalification Meeting", order: 2 },
  "3471215350": { label: "Active Research", order: 3 },
  "3471215351": { label: "Partner Identified", order: 4 },
  "3220310750": { label: "Teaser Sent", order: 5 },
  "3220310751": { label: "Intro to Partner", order: 6 },
  "3220310752": { label: "Intro Meeting Held", order: 7 },
  "3253863104": { label: "Partner Discussions", order: 8 },
...
export function computeMetrics(contacts, deals, callData, searchFilter, dateFilter = null) {
  ...
  // Interested replies from email (Instantly) and calls (HubSpot lead status)
  const interestedContacts = filtered.filter(
    (c) =>
      c.properties?.instantly_lead_status === "Interested" ||
      c.properties?.hs_lead_status === "interested"
  );

  // Interested replies from deal pipeline (Prequalification Meeting stage or beyond)
  const interestedStages = ["3220310749", "3471215350", "3471215351", "3220310750", "3220310751", "3220310752", "3253863104", "closedwon"];
  ...
  // Total active deals = open stages only (not Passed, not Closed Won, not Closed Lost)
  const openStages = ["3471208175", "3220310749", "3471215350", "3471215351", "3220310750", "3220310751", "3220310752", "3253863104"];
  const activeDeals = filteredDeals.filter((d) =>
    openStages.includes(d.properties?.dealstage)
  );

  // Teasers sent (historical) = any deal that reached Teaser Sent or beyond
  // If a deal is at or past Teaser Sent, a teaser was sent at some point
  const teaserAndBeyond = ["3220310750", "3220310751", "3220310752", "3253863104", "3253863103", "closedwon"];
  const teasersSent = filteredDeals.filter((d) =>
    teaserAndBeyond.includes(d.properties?.dealstage)
  );

  // Introductions made (historical) = any deal that reached Intro to Partner or beyond
  const introAndBeyond = ["3220310751", "3220310752", "3253863104", "3253863103", "closedwon"];
  const introductions = filteredDeals.filter((d) =>
    introAndBeyond.includes(d.properties?.dealstage)
  );

  // Pipeline detail table = active open deals only
  const activeStages = openStages;
  ...
  return {
    ...
    interestedFromCalls: filtered.filter(
      (c) => c.properties?.hs_lead_status === "interested"
    ).length,
    totalActiveDeals: activeDeals.length,
    teasersSent: teasersSent.length,
