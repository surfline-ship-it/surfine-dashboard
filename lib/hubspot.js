...
/** Stages shown in pipeline progression (order); excludes Re-Engage, Deferred Interest, Closed Lost */
export const PIPELINE_PROGRESSION_STAGES = [
  { id: "3471208175", label: "Pre-Qual Booking/Hunting Lead", rowClass: "pipeline-row-early" },
  { id: "3220310749", label: "Prequalification Meeting", rowClass: "pipeline-row-early" },
  { id: "3471215350", label: "Active Research", rowClass: "pipeline-row-early" },
  { id: "3471215351", label: "Partner Identified", rowClass: "pipeline-row-early" },
  { id: "3220310750", label: "Teaser Sent", rowClass: "pipeline-row-mid" },
  { id: "3220310751", label: "Intro Booking/Intro to Partner", rowClass: "pipeline-row-mid" },
  { id: "3220310752", label: "Intro Meeting Held", rowClass: "pipeline-row-late" },
  { id: "3253863104", label: "Partner Discussions", rowClass: "pipeline-row-late" },
  { id: "3253863103", label: "Passed", rowClass: "pipeline-row-passed" },
  { id: "closedwon", label: "Closed Won", rowClass: "pipeline-row-won" },
];
...
function buildPipelineProgression(filteredDeals) {
  return PIPELINE_PROGRESSION_STAGES.map(({ id, label, rowClass }) => {
    const inStage = filteredDeals.filter((d) => d.properties?.dealstage === id);
    return {
      stageId: id,
      label,
      rowClass,
...
export function computeMetrics(contacts, deals, callData, searchFilter, dateFilter = null) {
  ...
  return {
    ...
    uniqueCompaniesEmailed: emailedCompanies.size || companySet.size,
