  ...
  try {
    ...
    try {
      ...
      try {
        ...
        const broadDeals = await searchAll("deals", { properties: DEAL_PROPERTIES }, 10);
...
const PIPELINE_STAGE_IDS = Array.from(
...
const DEAL_PROPERTIES = [
  ...
  ...PIPELINE_STAGE_IDS.map((id) => `hs_date_entered_${id}`),
];
...
function getDealStatusUpdatedAt(deal) {
  ...
  if (stage === "3220310752" || stage === "3253863104") {
...
function buildPipelineProgression(filteredDeals) {
  ...
  const dealPayload = (d) => {
    ...
    const estimatedRevenue = formatFinancialLabel(d.properties?.estimated_revenue);
    const estimatedEbitda = formatFinancialLabel(d.properties?.estimated_ebitda);
    return {
      id: d.id,
