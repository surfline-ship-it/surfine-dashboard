...
function companyPartnerSearchFilterGroups(partner, searchName) {
  const interestedEq = { propertyName: "interested", operator: "EQ", value: "true" };
  const partnerEq = { propertyName: "partner", operator: "EQ", value: partner };
  if (!searchName) {
    return [{ filters: [interestedEq, partnerEq] }];
  }
  const values = searchNameHubSpotEquivalents(searchName);
  return values.map((v) => ({
    filters: [
...
export async function getInterestedCompanyCount(partner, searchName) {
  try {
    const companies = await searchAll("companies", {
      filterGroups: companyPartnerSearchFilterGroups(partner, searchName),
      properties: ["interested", "interest_source", "interest_date", "name", "domain"],
    }, 10);
    return companies.length;
  } catch (e) {
    const fallback = await searchAll("companies", {
      filterGroups: [{
        filters: [{ propertyName: "interested", operator: "EQ", value: "true" }],
      }],
