...
export function computeMetrics(contacts, deals, callData, searchFilter, dateFilter = null) {
  ...
  const isInterestedContact = (c) =>
    c.properties?.instantly_lead_status === "Interested" ||
    c.properties?.hs_lead_status === "interested";

  const interestedResponseDomains = new Set();
  filtered.forEach((c) => {
    if (!isInterestedContact(c)) return;
    const dom = emailDomainFromContact(c);
    if (dom) interestedResponseDomains.add(dom);
  });
  const interestedResponses = interestedResponseDomains.size;
