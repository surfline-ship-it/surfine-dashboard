...
export async function getPartnerDeals(partner, searchName) {
  ...
  try {
    ...
    // HubSpot can reject some OR filter payloads. Fall back to partner-only fetch,
    // then apply canonical search_name filtering in-app.
    if (!searchName) throw error;
    const deals = await searchAll("deals", {
      filterGroups: dealPartnerSearchFilterGroups(partner, null),
      properties,
    });
